...
import numpy as np
from .island_navigation_backup import IslandNavigationEnvironment #First change for another environmnent
...
class CartPoleEnv2(gym.Env):
    ...
    def __init__(self):
        self.hidden_env = IslandNavigationEnvironment() #Second change for another environmnent
        self.hidden_env.reset()

        #Here we should also return the observation and action space that are expected

        self.action_space = spaces.Discrete(6)
        self.observation_space = spaces.Box(shape=(6, 8), low=0, high=255, dtype=np.float32) #Third change for another environmnent (the shape)
