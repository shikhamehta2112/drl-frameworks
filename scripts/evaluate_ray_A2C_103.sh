...
echo
for fullfile in src/ray/experiments/cartpole/A2C/ray_a2c_cpu_cp103.yml; do 
    filename=$(basename -- "$fullfile")
    experiment="${filename%.*}"
    mkdir ${filename%.*}
    echo "--- STARTING EXPERIMENT ${experiment} --- "
    python src/ray/run_evaluation.py -f="src/ray/experiments/cartpole/A2C/ray_a2c_cpu_cp103.yml"
