...
echo
for fullfile in src/dopamine/experiments/cartpole/IQA/dopamine_IQ_cpu_cp103.gin; do
    filename=$(basename -- "$fullfile")
    experiment="${filename%.*}"
    mkdir ${filename%.*}
    echo "--- STARTING EXPERIMENT ${experiment} --- "
    python src/dopamine/run_evaluation.py --base_dir="src/results/dopamine/cartpole/IQA/" --gin_files="src/dopamine/experiments/cartpole/IQA/dopamine_IQ_cpu_cp103.gin"
